#!/usr/bin/env python3
"""
Run prior probe Groq/API experiments.

Tests LLM prior alignment by measuring T=0 argmax choices
without any interaction history.

Examples:
    python -m src.scripts.run_prior_probe_groq --preset groq-llama-3.1-8b-instant --runs 10
    
    # Mixed model experiment
    python -m src.scripts.run_prior_probe_groq --presets groq-llama-3.1-8b-instant groq-gpt-oss-20b --mixed
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Run prior probe Groq/API experiments"
    )
    ...
    parser.add_argument(
        ...
        choices=["groq"],
        default="groq",
        help="Backend for --model/--models (fixed to groq).",
    )
    ...
    # Output
    parser.add_argument("--outdir", type=str, default="results/prior_probe_groq")
    ...
    if args.presets:
        for preset in args.presets:
            ...
            if backend != "groq":
                parser.error(
                    f"Preset '{preset}' resolves to backend '{backend}', expected 'groq' for this runner."
                )
        ...
        if backend != "groq":
            parser.error(
                f"Preset '{args.preset}' resolves to backend '{backend}', expected 'groq' for this runner."
            )
        model_names.append(model_id)
    ...
    logger.info("=" * 60)
    logger.info("PRIOR PROBE GROQ EXPERIMENT")
    logger.info("=" * 60)
