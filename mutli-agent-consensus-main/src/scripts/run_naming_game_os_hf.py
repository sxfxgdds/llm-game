#!/usr/bin/env python3
"""
Run OS naming game ablations through Hugging Face hosted inference APIs.

This runner avoids local model loading and executes via HF endpoints.
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Run OS naming game ablations via Hugging Face Inference API"
    )

    parser.add_argument("--preset", type=str, default=None, help="HF API preset")
    parser.add_argument("--presets", nargs="+", default=None, help="One or more HF API presets")
    parser.add_argument("--model", type=str, default=None, help="Hugging Face model id")
    parser.add_argument("--models", nargs="+", default=None, help="One or more Hugging Face model ids")
    parser.add_argument(
        "--backend",
        type=str,
        choices=["hf_api"],
        default="hf_api",
        help="Backend used with --model/--models (fixed to hf_api).",
    )
    ...
    parser.add_argument("--outdir", type=str, default="results/os/naming_game_hf", help="Output directory")
    ...
    from src.experiments.naming_game_os_hf import NamingGameOSConfig, NamingGameOSHFExperiment

    logger.info("=" * 60)
    logger.info("OS NAMING GAME HF API EXPERIMENT")
    logger.info("=" * 60)
    ...
    if preset_inputs:
        ...
        for preset in preset_inputs:
            ...
            if backend != "hf_api":
                parser.error(
                    f"Preset '{preset}' resolves to backend '{backend}', expected 'hf_api'. "
                    "Use hfapi-* presets for this runner."
                )
    ...
    logger.info(f"Clients initialized in {time.time() - t0:.1f}s")
    ...
    experiment = NamingGameOSHFExperiment(clients, config)
    logger.info(f"Allowed names: {experiment.allowed_names}")
    results = experiment.run_all()
    total_elapsed = time.time() - total_start

    logger.info("=" * 60)
    logger.info("OS HF API EXPERIMENT COMPLETE")
    logger.info("=" * 60)
