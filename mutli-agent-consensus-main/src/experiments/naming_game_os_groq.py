...
class NamingGameOSExperiment:
    ...
    def __init__(self, clients, config: Optional[NamingGameOSConfig] = None):
        ...
        # Keep one fixed allowed-name list for all runs; only per-interaction order is shuffled.
        self._allowed_names_fixed = tuple(self.allowed_names)
        self._allowed_set_id = str(hash(tuple(sorted(self._allowed_names_fixed))))
    ...
    def _build_tokenizer_safe_labels(self) -> List[str]:
        ...
        for model_name in self.config.model_names:
            tok = getattr(self.clients[model_name], "tokenizer", None)
            if tok is not None:
                tokenizers.append(tok)
        candidates = self._load_candidates()

        # API clients (e.g., Groq/OpenAI) may not expose tokenizers. In that case,
        # use nonce-like candidate labels directly.
        if not tokenizers:
            if len(candidates) < self.config.n_names:
                raise ValueError(
                    f"Not enough labels in candidate list: need {self.config.n_names}, "
                    f"have {len(candidates)}."
                )
            if self.config.verbosity >= 1:
                logger.warning(
                    "No tokenizer available from selected client(s); "
                    "falling back to candidate labels without tokenizer parity checks."
                )
            return candidates[: self.config.n_names]
    ...
    def run_single(
        ...
        within_type_matches = []
        cross_type_matches = []
        allowed_base = list(self._allowed_names_fixed)
        ...
        for round_idx in range(self.config.n_rounds):
            ...
            for i, j in pairs:
                agent_i = agents[i]
                agent_j = agents[j]
                allowed_round = shared_allowed_for_round(allowed_base, rng)
                ...
                if not result_i.valid:
                    invalids += 1
                    err_i = result_i.meta.get("error")
                    if err_i:
                        logger.warning(
                            "    Choice error | run=%d round=%d agent=%d model=%s rate_limited=%s error=%s",
                            run_idx + 1,
                            round_idx + 1,
                            i,
                            model_i,
                            result_i.meta.get("rate_limited", False),
                            err_i,
                        )
                if not result_j.valid:
                    invalids += 1
                    err_j = result_j.meta.get("error")
                    if err_j:
                        logger.warning(
                            "    Choice error | run=%d round=%d agent=%d model=%s rate_limited=%s error=%s",
                            run_idx + 1,
                            ...
                            j,
                            model_j,
                            result_j.meta.get("rate_limited", False),
                            err_j,
                        )
                ...
                if trials_writer:
                    trials_writer.log({
                        ...
                        "allowed_set_id": self._allowed_set_id,
        ...
        return RunResult(
            ...
            config={
                "allowed_names": list(self._allowed_names_fixed),
    ...
    def run_condition(self, condition: str) -> List[RunResult]:
        ...
        if self.config.verbosity >= 1:
            ...
            logger.info(f"  Fixed allowed names (shared across runs): {list(self._allowed_names_fixed)}")
